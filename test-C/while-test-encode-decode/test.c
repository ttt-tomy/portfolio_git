...
int search(int ch ,int w ){
	...
	//while:式が成立する間は繰り返す ※0以外になっている間は繰り返すってこともできる
	while(i<w){
		//入力文字を1文字ずつ26回繰り返して当てはまるか判定
		//当てはまれば何文字目かをfindに代入、当てはまらなければ-1をfindに代入
		if(alphabet[i] == ch){
			find = i;
			break;
			//breakを使わない場合は条件式に(find==-1)を追加する
			//↑ifに当てはまらない場合は-1なので条件式から外れることになる
		}
		i++;
	}
